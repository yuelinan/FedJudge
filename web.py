...
from utils import StreamPeftGenerationMixin, printf
import utils
import copy
...
def evaluate(
    ...
    **kwargs,):
    ...
    if prompt_type == 'instruct':
        ...
        # PROMPT = prompt.chat_prompt(tokenizer,max_memory)
        PROMPT = prompt.chat_my_prompt(tokenizer,max_memory)
    else:
        raise Exception('not support')
    ...
    with torch.no_grad():
        ...
        # common 
            ...
            try:
                ...
                output = PROMPT.my_postprocess(inputs,output)
                history.append({
                    'input': inputs,
                    'output': output,
                })
    ...
    with gr.Row().style(equal_height=False):
        with gr.Column(variant="panel"):
            ...
            with input_component_column:
                ...
                topp = gr.components.Slider(minimum=0, maximum=1, value=1.0, label="Top p")
                topk = gr.components.Slider(minimum=0, maximum=100, step=1, value=50, label="Top k")
                ...
                max_new_token = gr.components.Slider(
                    minimum=1, maximum=2048, step=1, value=100, label="Max New Tokens"
                )
